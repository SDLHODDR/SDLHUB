...
const KRAActivity = () => {
  ...
  const listData = useMemo(() => {
    try {
      return normalizeRecords(kraActivityData).map((item, index) => ({
        ...
        KRA_DESC: getDisplayValue(
          item,
          ['KRA_DESC', 'kra_desc', 'KRA_MASTER_DESC', 'name', 'label'],
          '-'
        ),
        ACTT_DESC: getDisplayValue(
          item,
          ['ACTT_DESC', 'activity_desc', 'activityTitle', 'title', 'name'],
          '-'
        ),
  ...
  const masterOptions = useMemo(() => {
    return normalizeRecords(listKRAMasterData).map((item, index) => {
      const id = getDisplayValue(
        item,
        ['KRA_ID', 'ID', 'id', 'kra_id', 'KRAID', 'kraId'],
        index
      )
  ...
  const handleAddNewKRAMaster = useCallback(async typedText => {
    try {
      ...
      setListKRAMasterData(prev => [
        ...prev,
        { KRA_ID: newId, KRA_DESC: typedText }
      ])
      return newOption
    } catch (error) {
      ...
      setListKRAMasterData(prev => [
        ...prev,
        { KRA_ID: tempId, KRA_DESC: typedText }
      ])
                      ...
                      <div className='col-lg-4 col-md-6'>
                        ...
                        <SDLReactSelect
                          ...
                          onAddNew={async typedText => {
                            const newOption = await handleAddNewKRAMaster(
                              typedText
                            )
                      ...
                      <div className='col-lg-4 col-md-6'>
                        <div className='mb-3'>
                          <SDLInput
                            ...
                            onChange={e =>
                              handleFieldChange('ACTT_DESC', e.target.value)
                            }
                            maxLength={100}
                            error={errors.ACTT_DESC}
                          />
                        </div>
                      </div>
                    </div>
