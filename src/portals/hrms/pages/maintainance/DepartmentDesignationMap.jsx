...
import { getDepartmentDesignationMap, getDesignationsMaster, saveDepartmentDesignationMap } from "../../services/departmentService";
...
const normalizeRecords = (payload) => {
  ...
  for (const key of ["data", "records", "result", "items", "list", "rows", "departments", "department"]) {
    ...
    if (payload[key] && typeof payload[key] === "object") {
      for (const subKey of ["data", "records", "result", "items", "list", "rows", "departments", "designation", "designations"]) {
...
const DepartmentDesignationMap = () => {
  ...
  const toDesignationOption = (designation) => {
    const value = String(
      designation?.ID ?? designation?.id ?? designation?.DESIG_ID ?? designation?.DESI_ID ?? designation?.designationId ?? designation ?? "",
    );
    const label =
      designation?.DESIG_NAME || designation?.DESI_DESC || designation?.name || designation?.designation || designation?.DESIGNATION || String(designation || "");
  ...
  const listData = useMemo(() => {
    const normalized = rows.map((item, idx) => ({
      ...
      DEPT_CODE: item.DEPT_CODE ?? item.dept_code ?? item.code ?? item.DEPT_ID ?? item.id ?? "",
      DEPT_NAME: item.DEPT_NAME ?? item.dept_name ?? item.name ?? item.DEPT_DESC ?? item.dept_desc ?? item.department ?? "",
      DESIGNATIONS: Array.isArray(item.designations)
        ? item.designations
        : Array.isArray(item.DESIGNATIONS)
        ? item.DESIGNATIONS
        : item.designation_list || item.designations_list || item.designations || item.DESI_LIST || item.DESI_NAMES || [],
    }));

    if (!searchQuery.trim()) return normalized;
    const q = searchQuery.trim().toLowerCase();
    return normalized.filter((r) => (r.DEPT_NAME || "").toLowerCase().includes(q) || (r.DEPT_CODE || "").toString().toLowerCase().includes(q));
  }, [rows, searchQuery]);
  ...
  return (
    <>
      <div className="page-header">
        ...
        <BreadcrumbNav
          items={[{ text: "Home", link: portalHome }, { text: "Dept - Designation Map" }]}
        />
      </div>
            ...
            <div className="card-body">
              <div className="d-flex justify-content-between align-items-center mb-3">
                <div style={{ minWidth: 300 }}>
                  <SDLSearch value={searchQuery} onChange={setSearchQuery} placeholder="Search Department..." />
                </div>
              </div>
              ...
              {editingRow && (
                <div ref={editSectionRef} className="card mt-3">
                  <div className="card-body">
                    <h6 className="mb-3">Update Designations for: {editingRow.DEPT_NAME || editingRow.DEPT_CODE}</h6>
                    <div className="mb-3">
                      <Select
                        isMulti
                        options={designations}
                        value={selectedDesigs}
                        onChange={(v) => setSelectedDesigs(v || [])}
    ...
    control: (base) => ({
      ...
      minHeight: "40px",
                        ...
                        {selectedDesigs.map((selection) => (
                          ...
                          >
                            ...
                            <button
                              ...
                              aria-label="Remove"
                              onClick={() => setSelectedDesigs((current) => current.filter((item) => String(item.value) !== String(selection.value)))}
                            />
                          </span>
                        ))}
                      </div>
                    ...
                    {designations.length === 0 && (
                      <div className="text-muted small">Designation list not available.</div>
                    )}
