...
export const usePolicyHandler = ({
  ...
  const validateForm = useCallback(() => {
    ...
    } else if (formData.POLICY_NAME.length > 30) {
      newErrors.POLICY_NAME = 'Policy name cannot exceed 30 characters'
    } else if (!/^[A-Za-z0-9\s]+$/.test(formData.POLICY_NAME)) {
      newErrors.POLICY_NAME = 'Policy name must not contain special characters'
    }
    ...
    if (!formData.POLICY_DESC || !formData.POLICY_DESC.trim()) {
      newErrors.POLICY_DESC = 'Policy description is required'
    } else if (formData.POLICY_DESC.length > 200) {
      newErrors.POLICY_DESC = 'Policy description cannot exceed 200 characters'
    }

    if (!formData.doc && !formData.DOC_PATH)
      newErrors.doc = 'Document upload is required'
  ...
  const buildFormPayload = useCallback(() => {
    ...
    ;(formData.DIVISION_ID || []).forEach(id =>
      payload.append('DIVISION_ID[]', id)
    )
    ...
    async row => {
      ...
      try {
        ...
        if (response?.status) {
          ...
          notifyError(
            response?.message ||
              'Unable to load department/division for this policy.'
          )
        }
      } catch (error) {
