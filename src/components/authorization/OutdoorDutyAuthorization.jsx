...
import { getOutDoorDutyAuthResponse } from "../../store/eportal/ePortalAuthorizationOutDoorDutySlice";
import { getAuthroizationTaskCount } from "../../store/eportal/ePortalAuthorizationCountSlice";

const OutdoorDutyAuthorization = () => {
  const dispatch = useDispatch();
  const authODdata = useSelector((state) => state.eportalAuthOD.data);
  const loading = useSelector((state) => state.eportalAuthOD.loading);
  const [searchQuery, setSearchQuery] = useState("");
  const [listData, setListData] = useState([]);
const [refreshKey, setRefreshKey] = useState(0);

  useEffect(() => {
    dispatch(getOutDoorDutyAuthResponse({ task_id: 349 }));
  }, [dispatch, refreshKey]);
